...
def _fixed_padding(inputs, kernel_size, rate=1):  # pylint: disable=invalid-name
  ...
  padded_inputs = tf.compat.v1.pad(
...
class _LayersOverride(object):
  ...
  def __init__(self,
    ...
    self.regularizer = tf.compat.v1.keras.regularizers.l2(weight_decay)
  ...
  def _FixedPaddingLayer(self, kernel_size, rate=1):  # pylint: disable=invalid-name
    return tf.compat.v1.keras.layers.Lambda(
  ...
  def Conv2D(self, filters, kernel_size, **kwargs):  # pylint: disable=invalid-name
    ...
    if stride and kernel_size and stride > 1 and kernel_size > 1:
      ...
      def padded_conv(features):  # pylint: disable=invalid-name
        ...
        return tf.compat.v1.keras.layers.Conv2D(
            filters, kernel_size, **kwargs)(padded_features)
      return padded_conv
    else:
      return tf.compat.v1.keras.layers.Conv2D(filters, kernel_size, **kwargs)
  ...
  def Activation(self, *args, **kwargs):  # pylint: disable=unused-argument,invalid-name
    ...
    if self._conv_hyperparams:
      ...
      return tf.compat.v1.keras.layers.Lambda(tf.compat.v1.nn.relu, name=name)
  ...
  def Input(self, shape):  # pylint: disable=invalid-name
    ...
    input_tensor = tf.compat.v1.constant(0.0, shape=[default_batch_size] + default_shape)
  ...
  def MaxPooling2D(self, pool_size, **kwargs):  # pylint: disable=invalid-name
    ...
    """
    kwargs['padding'] = 'same'
    return tf.compat.v1.keras.layers.MaxPooling2D(pool_size, **kwargs)
  ...
  def __getattr__(self, item):
    return getattr(tf.compat.v1.keras.layers, item)
...
def resnet_v1_50(batchnorm_training,
  ...
  return tf.compat.v1.keras.applications.resnet.ResNet50(
...
def resnet_v1_101(batchnorm_training,
  ...
  layers_override = _LayersOverride(
      ...
      min_depth=min_depth,
      depth_multiplier=depth_multiplier)
  return tf.compat.v1.keras.applications.resnet.ResNet101(
      layers=layers_override, **kwargs)
...
def resnet_v1_152(batchnorm_training,
  ...
  layers_override = _LayersOverride(
      ...
      min_depth=min_depth,
      depth_multiplier=depth_multiplier)
  return tf.compat.v1.keras.applications.resnet.ResNet152(
      layers=layers_override, **kwargs)
...
def block_basic(x,
  ...
  bn_axis = 3 if tf.compat.v1.keras.backend.image_data_format() == 'channels_last' else 1
